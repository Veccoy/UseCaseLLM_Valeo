...
def test_home_page(client):
    ...
    assert b'TEXT SUMMARIZATION' in response.data  # Vérifie si le titre est présent

def test_summarization_success(client):
    """Test if summarization task works with a valid input"""
    input_text = "This is a long text that needs to be summarized by the model."
    response = client.post('/text-summarization', data={"inputtext_": input_text})
...
def test_summarization_empty_input(client):
    ...
    assert response.status_code == 200
    assert "ERROR: No inference made. Input text can't be empty!" in parsed_response.p.get_text()  # Check what the response
...
def test_summarization_large_input(client):
    ...
    response = client.post('/text-summarization', data={"inputtext_": large_input_text})
    parsed_response = BeautifulSoup(response.data, 'html.parser')
